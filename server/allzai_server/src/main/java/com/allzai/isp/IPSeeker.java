...
public class IPSeeker {
	...
	private static final String CONFIG_FILE = "/isp.properties";
	...
	public IPSeeker() {
		...
		try {
			properties.load(IPSeeker.class.getResourceAsStream(CONFIG_FILE));
		} catch (IOException ex) {
			ex.printStackTrace();
		}
		
		String fileName = properties.getProperty("IP_FILE");
		String dir = properties.getProperty("INSTALL_DIR");
		...
		for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
			...
			if (temp != -1) {
				...
				if (ipLoc.getCountry().indexOf(s) != -1
					...
					// 得到起始IP
					readIP(offset - 4, b4);
					entry.beginIp = IspUtil.getIpStringFromBytes(b4);
					// 得到结束IP
					readIP(temp, b4);
					entry.endIp = IspUtil.getIpStringFromBytes(b4);
					// 添加该记录
					ret.add(entry);
				}
			}
	...
	public String getCountry(byte[] ip) {
		...
		// 保存ip，转换ip字节数组为字符串形式
		String ipStr = IspUtil.getIpStringFromBytes(ip);
	...
	public String getCountry(String ip) {
		return getCountry(IspUtil.getIpByteArrayFromString(ip));
	}
	...
	public String getArea(String ip) {
		return getArea(IspUtil.getIpByteArrayFromString(ip));
	}
	...
	private String readString(long offset) {
		try {
			...
			if (i != 0)
				return IspUtil.getString(buf, 0, i, "GBK");
		} catch (IOException e) {
			e.printStackTrace();
